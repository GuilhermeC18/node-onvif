	...
	this.udp.on('message', (buf, device_info) => {
		this.OnvifSoap.parse(buf.toString(), (error, result) => {
			if(error) {
				...
				var probe_match = result['Body']['ProbeMatches']['ProbeMatch'];
				var urn = probe_match['EndpointReference']['Address'];
				var xaddrs = probe_match['XAddrs'].split(/\s+/);
				if(urn && xaddrs.length > 0) {
					if(!this.devices[urn]) {
						var scopes = probe_match['Scopes'].split(/\s+/);
						var name = '';
						var hardware = '';
						var location = '';
						scopes.forEach((s) => {
