...
*	J. UDALL - added ability to log SOAP messages
*	- method - a method that will be invoked for every SOAP message sent/received
* ---------------------------------------------------------------- */
Onvif.prototype.enableLogging = function(method) {
	this._OnvifSoap.enableLogging(method)
}
...
* ---------------------------------------------------------------- */
Onvif.prototype.startProbe = function(options, callback) {
	let promise = new Promise((resolve, reject) => {
		...
		//  J. UDALL.  CHANGE FROM ORIGINAL CODE TO SPECIFY NIC
		let bindAddress = '0.0.0.0'
		if (options == null || options.device == null){
			throw("Mandatory options.device is missing")
		} else {
				const os = require('os')
				...
				if (options.device in interfaces) {
					...
					throw("NIC '" + options.device + "' not found or unplugged")
		}
		this._udp.bind(null,bindAddress,() => {
