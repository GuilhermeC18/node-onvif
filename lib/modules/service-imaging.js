...
OnvifServiceImaging.prototype.getImagingSettings = function(params, callback){
	...
	let promise = new Promise((resolve, reject) => {
		...
		soap_body +=   '<timg:ReferenceToken>' + params['VideoSourceToken'] + '</timg:ReferenceToken>';
		soap_body += '</timg:GetImagingSettings>';
		let soap = this._createRequestSoap(soap_body);
...
OnvifServiceImaging.prototype.setImagingSettings = function(params, callback){
	let promise = new Promise((resolve, reject) => {
		...
		if ('IrCutFilter' in params){
			if(err_msg = mOnvifSoap.isInvalidValue(params['IrCutFilter'], 'string')) {
				reject(new Error('The "mode" property was invalid: ' + err_msg));
				return;
			}
		}
        ...
        if ('IrCutFilter' in params)
            soap_body +=     '<tt:IrCutFilter>'+ params['IrCutFilter'] + '</tt:IrCutFilter>';
        
        if ('Iris' in params)
            soap_body +=     '<tt:Exposure><tt:Mode>MANUAL</tt:Mode><tt:Iris>'+ params['Iris'] + '</tt:Iris></tt:Exposure>';   
		
		soap_body +=     '<tt:ForcePersistance>true</tt:ForcePersistance>';   
		soap_body +=   '</timg:ImagingSettings>';
		soap_body += '</timg:SetImagingSettings>';
		let soap = this._createRequestSoap(soap_body);
