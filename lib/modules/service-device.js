...
OnvifServiceDevice.prototype._createRequestSoap = function(body,omitAuthorization) {
...
* Method: getDateTime()
* ---------------------------------------------------------------- */
OnvifServiceDevice.prototype.getDateTime = function() {
...
OnvifServiceDevice.prototype.getCapabilities = function(callback) {
	let promise = new Promise((resolve, reject) => {
		...
		soap_body += '</tds:GetCapabilities>';
		let soap = this._createRequestSoap(soap_body,true);
...
/* ------------------------------------------------------------------
* Method: setDiscoveryMode(mode, callback)
* - mode | String | required | 'Discoverable' or 'NonDiscoverable'
*
* }* ---------------------------------------------------------------- */
OnvifServiceDevice.prototype.setDiscoveryMode = function(mode, callback) {
	let promise = new Promise((resolve, reject) => {
		let err_msg = '';
		if(err_msg = mOnvifSoap.isInvalidValue(mode, 'string')
		|| (mode != 'Discoverable' && mode != 'NonDiscoverable')) {
			reject(new Error('The value of "mode" was invalid: ' + err_msg));
			return;
		}
	
		let soap_body = '<tds:SetDiscoveryMode xmlns="http://www.onvif.org/ver10/device/wsdl">';
		soap_body +=   		'<DiscoveryMode>' + mode + '</DiscoveryMode>';
		soap_body += '</tds:SetDiscoveryMode>';
		let soap = this._createRequestSoap(soap_body);
		mOnvifSoap.requestCommand(this.oxaddr, 'SetDiscoveryMode', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
...
/* ------------------------------------------------------------------
* Method: setNetworkInterfaces([callback])
* ---------------------------------------------------------------- */
OnvifServiceDevice.prototype.setNetworkInterfaces = function(info,callback) {
	//info.data.GetNetworkInterfacesResponse.NetworkInterfaces.$.token
	//info.data.GetNetworkInterfacesResponse.NetworkInterfaces.IPv4.enabled = true
	//info.data.GetNetworkInterfacesResponse.NetworkInterfaces.IPv4.Config.DHCP = false
	//info.data.GetNetworkInterfacesResponse.NetworkInterfaces.IPv4.Config.Manual.Address = req.body.newip
	//info.data.GetNetworkInterfacesResponse.NetworkInterfaces.IPv4.Config.Manual.PrefixLength = "24"
	let promise = new Promise((resolve, reject) => {
		let soap_body = '';
		soap_body += '<tds:SetNetworkInterfaces xmlns="http://www.onvif.org/ver10/device/wsdl">';
		soap_body += 	'<InterfaceToken>' + info.interface.data.GetNetworkInterfacesResponse.NetworkInterfaces.$.token + '</InterfaceToken>'
		soap_body += 	'<NetworkInterface>'
		soap_body += 		'<Enabled xmlns="http://www.onvif.org/ver10/schema">true</Enabled>'
		//soap_body += 		'<MTU>1500</MTU>'
		soap_body += 		'<IPv4 xmlns="http://www.onvif.org/ver10/schema">'
		soap_body += 			'<Enabled>true</Enabled>'
		if (info.dhcp){
			soap_body += 		'<DHCP>true</DHCP>'
		} else {
			soap_body += 		'<Manual>'
			soap_body += 			'<Address>' + info.ip + '</Address>'
			soap_body += 			'<PrefixLength>24</PrefixLength>'
			soap_body += 		'</Manual>'
			soap_body += 		'<DHCP>false</DHCP>'
		}
		soap_body += 		'</IPv4>'
		soap_body += 	'</NetworkInterface>'
		soap_body += '</tds:SetNetworkInterfaces>';
		let soap = this._createRequestSoap(soap_body);
		mOnvifSoap.requestCommand(this.oxaddr, 'SetNetworkInterfaces', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
