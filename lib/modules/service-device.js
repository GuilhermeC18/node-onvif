...
OnvifServiceDevice.prototype.setNetworkInterfaces = function(info,callback) {
	...
	let promise = new Promise((resolve, reject) => {
		...
		soap_body += 	'<tt:InterfaceToken>' + info.interface.data.GetNetworkInterfacesResponse.NetworkInterfaces.$.token + '</tt:InterfaceToken>'
		soap_body += 	'<tt:NetworkInterface>'
		soap_body += 		'<tt:Enabled>true</tt:Enabled>'
		soap_body += 		'<tt:MTU>1500</tt:MTU>'
		soap_body += 		'<tt:IPv4>'
		soap_body += 			'<tt:Enabled>true</tt:Enabled>'
		soap_body += 			'<tt:Manual>'
		soap_body += 				'<tt:Address>' + info.ip + '</tt:Address>'
		soap_body += 				'<tt:PrefixLength>24</tt:PrefixLength>'
		soap_body += 			'</tt:Manual>'
		soap_body += 			'<tt:DHCP>false</tt:DHCP>'
		soap_body += 		'</tt:IPv4>'
		soap_body += 	'</tt:NetworkInterface>'
		soap_body += '</tds:SetNetworkInterfaces>';
		let soap = this._createRequestSoap(soap_body);
