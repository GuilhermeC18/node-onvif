...
OnvifServiceSearch.prototype.findRecordings = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		...
		if(err_msg = mOnvifSoap.isInvalidValue(params['Content'], 'string')) {
			reject(new Error('The "Content" property was invalid: ' + err_msg));
			return;
		}

		if(err_msg = mOnvifSoap.isInvalidValue(params['MaximumRetentionTime'], 'integer')) {
			reject(new Error('The "MaximumRetentionTime" property was invalid: ' + err_msg));
			return;
		}
