...
*
* J. Udall
*	- method	- a logger function that will be invoked for every SOAP message
* ---------------------------------------------------------------- */
OnvifSoap.prototype.enableLogging = function(method) {
	mLogger = method
}
	...
	let promise = new Promise((resolve, reject) => {
		...
		this._request(oxaddr, soap).then((res) => {
			...
			if(fault) {
				...
				if (typeof fault == 'object' && fault._)
					err = new Error(fault._)
				reject(err);
			} else {
				...
				if(parsed) {
					...
					//  JU: This sucks!  The Arecont Vision camera provides a RemoveVideoEncoderConfigurationResponse
					//  rather than a RemoveVideoSourceConfigurationResponse
					//  So look for this abberation
					if (method_name == 'RemoveVideoSourceConfiguration'){
						parsed = this._parseResponseResult('RemoveVideoEncoderConfiguration', result)
						if (parsed){
							let res = {
								'soap'     : xml,
								'formatted': mHtml ? mHtml.prettyPrint(xml, {indent_size: 2}) : '',
					...
					if(xml) {
						this.parse(xml).then((parsed) => {
							...
							try {
								...
								if (parsed['Body']['Fault']['Detail']){
									msg += ` (${parsed['Body']['Fault']['Detail']['Text']})`
								}
							} catch(e) {}
							if(msg) {
