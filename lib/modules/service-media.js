...
* {
*   'OSDToken': 'OSD1','
*	'VideoSourceConfigurationToken': 'VS1',
...
OnvifServiceMedia.prototype.createOSD = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		...
		soap_body += '<trt:OSD>'
		soap_body += '<trt:token>' + params['OSDToken'] + '</trt:token>';
		...
		soap_body += '<trt:Position>LowerLeft</trt:Position>'
		soap_body += '<trt:TextString><trt:Type>Plain</trt:Type><trt:PlainText>' + params['text'] + '</trt:PlainText></trt:TextString>'
		soap_body += '</trt:OSD>'
		soap_body += '</trt:CreateOSD>';
		let soap = this._createRequestSoap(soap_body);
