...
OnvifServiceMedia.prototype.setVideoEncoderConfiguration = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		...
		try {
			...
			soap_body +=   		'<tt:UseCount xmlns="http://www.onvif.org/ver10/schema">' + params.Configuration.UseCount + '</tt:UseCount>';  // deprecated in Media 2 as read-only
...
/* ------------------------------------------------------------------
* Method: removeAudioEncoderConfiguration(params[, callback])
* - params:
*	- ProfileToken | Object | required | a token of the profile
*
* {
...
* }
* ---------------------------------------------------------------- */
OnvifServiceMedia.prototype.removeAudioEncoderConfiguration = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		let err_msg = '';
		...
		let soap_body = '';
		try {
			soap_body += '<trt:RemoveAudioEncoderConfiguration xmlns="http://www.onvif.org/ver10/media/wsdl">';
			soap_body +=   '<ProfileToken>' + params.ProfileToken + '</ProfileToken>';
			soap_body += '</trt:RemoveAudioEncoderConfiguration>';
		} catch (err){
			reject(new Error('Missing required configuration parammeters'));
			return;			
		}
		...
		mOnvifSoap.requestCommand(this.oxaddr, 'RemoveAudioEncoderConfiguration', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
...
/* ------------------------------------------------------------------
* Method: removeAudioSourceConfiguration(params[, callback])
* - params:
*	- ProfileToken | Object | required | a token of the profile
*
* {
...
* }
* ---------------------------------------------------------------- */
OnvifServiceMedia.prototype.removeAudioSourceConfiguration = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		let err_msg = '';
		...
		let soap_body = '';
		try {
			soap_body += '<trt:RemoveAudioSourceConfiguration xmlns="http://www.onvif.org/ver10/media/wsdl">';
			soap_body +=   '<ProfileToken>' + params.ProfileToken + '</ProfileToken>';
			soap_body += '</trt:RemoveAudioSourceConfiguration>';
		} catch (err){
			reject(new Error('Missing required configuration parammeters'));
			return;			
		}
		...
		mOnvifSoap.requestCommand(this.oxaddr, 'RemoveAudioSourceConfiguration', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
...
/* ------------------------------------------------------------------
* Method: setAudioEncoderConfiguration(params[, callback])
* - params:
*   - ConfigurationToken | String | required | 
...
*
* {
*   'ConfigurationToken': 'audio_encoder_token'
* }
* ---------------------------------------------------------------- */
OnvifServiceMedia.prototype.setAudioEncoderConfiguration = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		let err_msg = '';
		if(err_msg = mOnvifSoap.isInvalidValue(params, 'object')) {
			reject(new Error('The value of "params" was invalid: ' + err_msg));
		...
		let soap_body = '';
		soap_body += '<trt:SetAudioEncoderConfiguration>';
		soap_body += `<trt:Configuration token="${params['ConfigurationToken']}">`;
		if (params['Encoding'])
			soap_body +=	'<tt:Encoding>' + params['Encoding'] + '</tt:Encoding>'
		if (params['Bitrate'])
			soap_body +=	'<tt:Bitrate>' + params['Bitrate'] + '</tt:Bitrate>'
		if (params['SampleRate'])
			soap_body +=	'<tt:SampleRate>' + params['SampleRate'] + '</tt:SampleRate>'
		soap_body += '<tt:Multicast>';
		soap_body += '<tt:Address>';
		soap_body += '<tt:Type>IPv4</tt:Type>';
		soap_body += '<tt:IPv4Address>0.0.0.0</tt:IPv4Address>';
		soap_body += '<tt:IPv6Address></tt:IPv6Address>';
		soap_body += '</tt:Address>';
		soap_body += '<tt:Port>0</tt:Port>';
		soap_body += '<tt:TTL>5</tt:TTL>';
		soap_body += '<tt:AutoStart>false</tt:AutoStart>';
		soap_body += '</tt:Multicast>';
		soap_body += '<tt:SessionTimeout>PT60S</tt:SessionTimeout>';
		soap_body += '</trt:Configuration>';
		soap_body += '<trt:ForcePersistence>true</trt:ForcePersistence>';
		soap_body += '</trt:SetAudioEncoderConfiguration>';

		let soap = this._createRequestSoap(soap_body);
		mOnvifSoap.requestCommand(this.oxaddr, 'SetAudioEncoderConfiguration', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
...
/* ------------------------------------------------------------------
* Method: getAudioOutputs([, callback])
* ---------------------------------------------------------------- */
OnvifServiceMedia.prototype.getAudioOutputs = function(callback) {
	let promise = new Promise((resolve, reject) => {
	
		let soap_body = '';
		soap_body += '<trt:GetAudioOutputs>';
		soap_body += '</trt:GetAudioOutputs>';

		let soap = this._createRequestSoap(soap_body);
	
		mOnvifSoap.requestCommand(this.oxaddr, 'GetAudioOutputs', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
...
/* ------------------------------------------------------------------
* Method: getAudioOutputConfigurations([, callback])
* ---------------------------------------------------------------- */
OnvifServiceMedia.prototype.getAudioOutputConfigurations = function(callback) {
	let promise = new Promise((resolve, reject) => {
	
		let soap_body = '';
		soap_body += '<trt:GetAudioOutputConfigurations>';
		soap_body += '</trt:GetAudioOutputConfigurations>';

		let soap = this._createRequestSoap(soap_body);
	
		mOnvifSoap.requestCommand(this.oxaddr, 'GetAudioOutputConfigurations', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
...
/* ------------------------------------------------------------------
* Method: setAudioOutputConfiguration(params[, callback])
* - params:
*   - ConfigurationToken | String | required | 
*   - OutputToken | String | required | 
*	- OutputLevel | integer | required
...
* }
* ---------------------------------------------------------------- */
OnvifServiceMedia.prototype.setAudioOutputConfiguration = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		let err_msg = '';
		if(err_msg = mOnvifSoap.isInvalidValue(params, 'object')) {
			reject(new Error('The value of "params" was invalid: ' + err_msg));
		...
		if(err_msg = mOnvifSoap.isInvalidValue(params['OutputToken'], 'string')) {
			reject(new Error('The "OutputToken" property was invalid: ' + err_msg));
			return;
		}
		if(err_msg = mOnvifSoap.isInvalidValue(params['OutputLevel'], 'integer')) {
			reject(new Error('The "OutputLevel" property was invalid: ' + err_msg));
			return;
		}
	
		let soap_body = '';
		soap_body += '<trt:SetAudioOutputConfiguration>';
		soap_body += '<trt:ConfigurationToken>' + params['ConfigurationToken'] + '</trt:ConfigurationToken>';
		soap_body += '<trt:OutputToken>' + params['OutputToken'] + '</trt:OutputToken>';
		soap_body += '<tt:OutputLevel>' + params['OutputLevel'] + '</tt:OutputLevel>'
		soap_body += '</trt:SetAudioOutputConfiguration>';

		let soap = this._createRequestSoap(soap_body);
	
		mOnvifSoap.requestCommand(this.oxaddr, 'SetAudioOutputConfiguration', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
...
OnvifServiceMedia.prototype.getOSDs = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		...
		if (params['VideoSourceToken'])
 			soap_body += 	'<tt:ConfigurationToken>' + params['VideoSourceToken'] + '</tt:ConfigurationToken>'
		soap_body += '</trt:GetOSDs>';
		let soap = this._createRequestSoap(soap_body);
...
OnvifServiceMedia.prototype.getOSDOptions = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		...
		soap_body += '<trt:ConfigurationToken>' + params['VideoSourceToken'] + '</trt:ConfigurationToken>';
		soap_body += '<tt:ConfigurationToken>' + params['VideoSourceToken'] + '</tt:ConfigurationToken>';
		soap_body += '</trt:GetOSDOptions>';
		let soap = this._createRequestSoap(soap_body);
...
*	'Text': [optional] 'this is text'
*	'DateFormat': [optional] YYY-MM-DD
*	'TimeFormat': [optional] hh:mm:ss
* }
* ---------------------------------------------------------------- */
OnvifServiceMedia.prototype.createOSD = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		...
		soap_body += 	'<trt:OSD token="' + params['Token'] + '">'
		soap_body += 		'<tt:VideoSourceConfigurationToken>' + params['VideoSourceConfigurationToken'] + '</tt:VideoSourceConfigurationToken>';
		soap_body +=		'<tt:Type>Text</tt:Type>'
		soap_body += 		'<tt:Position>'
		soap_body += 			'<tt:Type>' + params['Position'] + '</tt:Type>'
		soap_body += 		'</tt:Position>';
		soap_body +=		'<tt:TextString>';
		if (params['DateFormat']){
			if (params['TimeFormat']){
				soap_body +=	'<tt:Type>DateAndTime</tt:Type>'
				soap_body +=	'<tt:DateFormat>' + params['DateFormat'] + '</tt:DateFormat>'
				soap_body +=	'<tt:TimeFormat>' + params['TimeFormat'] + '</tt:TimeFormat>'
			} else {
				soap_body +=	'<tt:Type>Date</tt:Type>'
				soap_body +=	'<tt:DateFormat>' + params['DateFormat'] + '</tt:DateFormat>'
			}
		} else if (params['TimeFormat']){
			soap_body +=		'<tt:Type>Time</tt:Type>'
			soap_body +=		'<tt:TimeFormat>' + params['TimeFormat'] + '</tt:TimeFormat>'
		} else if (params['Text']){
			soap_body +=		'<tt:Type>Plain</tt:Type>'
			soap_body += 		'<tt:PlainText>' + params['Text'] + '</tt:PlainText>';
		} else {
			soap_body +=		'<tt:Type>Plain</tt:Type>'
			soap_body +=		'<tt:PlainText></tt:PlainText>'			
		}
		soap_body +=		'</tt:TextString>'
		soap_body +=	'</trt:OSD>'
...
OnvifServiceMedia.prototype.deleteOSD = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		...
		soap_body += '<tt:OSDToken>' + params['Token'] + '</tt:OSDToken>';
		soap_body += '<trt:OSDToken>' + params['Token'] + '</trt:OSDToken>';
		soap_body += '</trt:DeleteOSD>';
		let soap = this._createRequestSoap(soap_body);
